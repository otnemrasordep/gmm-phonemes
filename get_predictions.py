...
def get_predictions(mu, s, p, X):
	# MAX_LIKELIHOOD likelihood matrix for X over each cluster in k given model parameters
    # Returns the joint max likelihood estimate for each of the clusters
	"""
	...
	for i in range(k):
		...
		# b_i = P(b|x_i), with Bayes Rule
		# Probability that every point belongs to each cluster k
